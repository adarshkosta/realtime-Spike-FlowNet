...
parser.add_argument('--savedir', type=str, metavar='DATASET', default='results_dt1',
...
parser.add_argument('--render', dest='render', action='store_true',
                    help='evaluate model on validation set')
parser.add_argument('--testenv', default='indoor_flying1', type=str, metavar='N',
...
class Train_loading(Dataset):
    ...
    def __getitem__(self, index):
        if index + 100 < self.length and index > 100:
            ...
            if torch.max(aaa)>0 and torch.max(bbb)>0 and torch.max(ccc)>0 and torch.max(ddd) and torch.max(ee)>0 and torch.max(ff)>0:
...
def train(train_loader, model, optimizer, epoch, train_writer):
    global n_iter, args, event_interval, image_resize, sp_threshold, spike_repeat
    ...
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            ...
            kbar.update(batch_idx, values=[('BPS', 1/batch_time.avg),
...
def validate(test_loader, model, epoch, output_writers):
    ...
    if 'outdoor' in testenv:
        ...
        start1 = 0
        stop1 = len(test_loader)
        start2 = 0
        stop2 = len(test_loader)
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            if (start1 < i < stop1) or (start2 < i < stop2):
                input_representation = torch.zeros(former_inputs_on.size(0), batch_size_v, image_resize, image_resize, former_inputs_on.size(3)).float()
                ...
                # compute output
                inp.append(input_representation.max())
                input_representation = input_representation.to(device)
                output = model(input_representation.type(torch.cuda.FloatTensor), image_resize, sp_threshold)
                ...
                pred_flow[:, :, 0] = cv2.resize(np.array(output_temp[0, 0, :, :]), (scale * image_resize, scale * image_resize), interpolation=cv2.INTER_LINEAR)
                pred_flow[:, :, 1] = cv2.resize(np.array(output_temp[0, 1, :, :]), (scale * image_resize, scale * image_resize), interpolation=cv2.INTER_LINEAR)
                ...
                if 'outdoor' in testenv:
                    AEE, percent_AEE, n_points, AEE_sum_temp, AEE_gt, AEE_sum_temp_gt = flow_error_dense(gt_flow, pred_flow, (torch.sum(torch.sum(torch.sum(input_representation, dim=0), dim=0), dim=2)).cpu(), is_car=True)
                else:
                ...
                if i%100 == 0 and i>10:
                    print(i, iters, time.time()-base_time)
                    print('-------------------------------------------------------')
...
def main():
    global args, best_EPE, image_resize, event_interval, spiking_ts, device, sp_threshold, spike_repeat
    ...
    for epoch in range(args.start_epoch, args.epochs):
        scheduler.step()
        log('Epoch: ' + str(epoch+1) + ' / ' + str(args.epochs))
