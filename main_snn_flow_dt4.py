...
parser.add_argument('--threads', default=8, type=int, metavar='N',
...
class Train_loading(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        # gray image re-size
        self.length =  28583 #d_set['davis']['left']['image_raw'].shape[0]
        d_set = None
...
def train(train_loader, model, optimizer, epoch, train_writer):
    ...
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            ...
            kbar.update(batch_idx, values=[('T', T),
                                           ('BPS', 1/batch_time.avg),
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            if (start1 < i < stop1) or (start2 < i < stop2):
                ...
                temp_AEE = temp_AEE + args.div_flow * AEE
                temp_AEE_gt = temp_AEE_gt + args.div_flow * AEE_gt
                ...
                if i%100 == 0 and i>10:
                    ...
                    print('-------------------------------------------------------')
                    print('Mean AEE: {:.2f}, Mean AEE_gt: {:.2f}'
                                  .format(temp_AEE / 100, temp_AEE_gt / 100))
                    print('-------------------------------------------------------')
                    temp_AEE = 0.
                    temp_AEE_gt = 0.
